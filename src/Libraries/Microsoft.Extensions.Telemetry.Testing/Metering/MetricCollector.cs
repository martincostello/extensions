    ...
    where T : struct
{
    ...
    private static readonly HashSet<Type> _supportedDimensionValueTypes = new()
    {
        typeof(int),
        typeof(byte),
        typeof(short),
    ...
    private readonly Dictionary<string, MetricValue<T>> _metrics = new();
    private readonly List<Waiter> _waiters = new();
    private readonly TimeProvider _timeProvider;
    private AggregationType _aggregationType;
    private bool _disposed;
    private Instrument? _instrument;
    private MetricValue<T>? _latestMetricValueUpdated;
    ...
    public MetricCollector(Instrument instrument, TimeProvider? timeProvider = null)
        : this(timeProvider)
    {
        _ = Throw.IfNull(instrument);

        if (instrument is not Instrument<T> and not ObservableInstrument<T>)
        {
#pragma warning disable S2302 // "nameof" should be used
            throw new InvalidOperationException("Invalid instrument type, must be an 'Instrument<T>'");
#pragma warning restore S2302 // "nameof" should be used
        }

        _instrument = instrument;
        _aggregationType = GetAggregationType(instrument);
    ...
    /// <param name="meterScope">The scope of the meter that publishes the instrument to record.</param>
    ...
    /// </remarks>
    public MetricCollector(string meterName, object? meterScope, string instrumentName, TimeProvider? timeProvider = null)
        : this(timeProvider)
    {
        _ = Throw.IfNullOrEmpty(meterName);
        _ = Throw.IfNullOrEmpty(instrumentName);

        Initialize(instrument => object.Equals(instrument.Meter.Scope, meterScope) && instrument.Meter.Name == meterName && instrument.Name == instrumentName);
    }
    ...
    private MetricCollector(TimeProvider? timeProvider)
    {
        if (!_supportedTs.Contains(typeof(T)))
        {
            ...
            throw new InvalidOperationException($"MetricCollector can only be created for types {str}.");
        }
    ...
    public IReadOnlyList<CollectedMeasurement<T>> SnapshotMeasurements(bool clear = false)
    {
        lock (_measurements)
        {
            ThrowIfDisposed();
    ...
    /// <summary>
    /// Gets a snapshot of metrics collected by this collector.
    /// </summary>
    /// <param name="clear">Setting this to <see langword="true"/> will atomically clear the set of accumulated measurements.</param>
    /// <returns>The metric values recorded by this collector.</returns>
    public IReadOnlyCollection<MetricValue<T>> SnapshotMetricValues(bool clear = false)
    {
        lock (_measurements)
        {
            ThrowIfDisposed();
            ...
            // copy the values
            var values = new MetricValue<T>[_metrics.Count];
            int index = 0;
            foreach (var metric in _metrics.Values)
            {
                values[index++] = new MetricValue<T>(metric.Value, metric.Tags, metric.NumWrites);
            }
    ...
    /// </summary>
    public CollectedMeasurement<T>? LatestMeasurementTaken
    {
        get
        {
            lock (_measurements)
    ...
    /// <summary>
    /// Gets the latest metric value that was updated.
    /// </summary>
    public MetricValue<T>? LatestMetricValueUpdated
    {
        get
        {
            lock (_measurements)
            {
                ThrowIfDisposed();
                return _latestMetricValueUpdated;
            }
        }

        private set => _latestMetricValueUpdated = value;
    }
    ...
    /// <summary>
    /// Scan all registered recorded instruments.
    /// </summary>
    public void RecordObservableInstruments()
    {
        if (_instrument is ObservableInstrument<T>)
        {
            _meterListener.RecordObservableInstruments();
        }
    }

    private static AggregationType GetAggregationType(Instrument instrument)
    {
    ...
    /// <summary>
    /// Creates a dictionary key by combining the tags in a consistent way.
    /// </summary>
    private static string CreateKey(ReadOnlySpan<KeyValuePair<string, object?>> tags, out KeyValuePair<string, object?>[] sortedTags)
    {
        ...
        foreach (var kvp in sortedTags)
        {
            ...
            else
            {
                ...
                if (valueType == typeof(string) || (!valueType.IsArray && _supportedDimensionValueTypes.Contains(valueType)))
                {
                    _ = keyBuilder.Append(kvp.Value);
                }
                else if (valueType.IsArray && _supportedDimensionValueTypes.Contains(valueType.GetElementType()!))
                {
    ...
    private void OnMeasurementRecorded(Instrument instrument, T measurement, ReadOnlySpan<KeyValuePair<string, object?>> tags, object? state)
    {
        ...
        lock (_measurements)
        {
            ...
            // create/update the metric value
            MetricValue<T>? metricValue = null;
            switch (_aggregationType)
            {
                case AggregationType.Save:
                case AggregationType.SaveOrUpdate:
                    if (_metrics.TryGetValue(key, out metricValue))
                    {
                        metricValue.Update(measurement);
                    }
                    else
                    {
                        metricValue = new MetricValue<T>(measurement, sortedTags);
            ...
            if (metricValue != null)
            {
                LatestMetricValueUpdated = new MetricValue<T>(metricValue.Value, metricValue.Tags, metricValue.NumWrites);
            }
